import { footerLinks, manufacturers } from '@/constants'
import Image from 'next/image'
import Link from 'next/link'
import React from 'react'

const Footer = () => {
  return (
    <footer className='mt-5 flex flex-col text-black-100 border-t border-gray-100 '>
      <div className='flex max-md:flex-col flex-wrap justify-between gap-5 px-6 sm:px-16 py-10'>
        <div className='flex flex-col justify-start items-start gap-6'>
          <Image src={"/logo.svg"} width={118} height={18} className='object-contain' alt='logo' />
            <p className='text-gray-700 text-base'>Car hub 2024 <br />all Right Reserverd &copy;</p>          
        </div>
        <div className='footer__links'>
          {footerLinks.map((link) => (
            <div key={link.title} className='footer__link'>
            <h3 className='font-bold'>{link.title}</h3>
            {link.links.map((lieo) => (
              <Link key={lieo.title} href={lieo.url} className='text-gray-500'>
                {lieo.title}
              </Link>
            ))}
            </div>
          ))}
        </div>
      </div>
        <div className='flex justify-between items-center  flex-wrap mt-10 border-t border-gray-100 px-6 sm:px-16 py-10'>
        <p>@2024 Carhub all Right Reserverd </p>
          <div className='footer__copyrights-link'>
            <Link href={"/"} className='text-gray-500'>Privacy Policy</Link>
            <Link href={"/"} className='text-gray-500'>Terms of Use</Link>
          </div>
        </div>
    </footer>
  )
}

export default Footer